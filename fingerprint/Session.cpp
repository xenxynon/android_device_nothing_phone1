...
Session::Session(fingerprint_device_t *device, int32_t userId,
  ...
  std::string path =
      ::android::base::StringPrintf("/data/vendor_de/%d/fpdata/", mUserId);
...
ndk::ScopedAStatus
Session::authenticate(int64_t operationId,
                      std::shared_ptr<ICancellationSignal> *out) {
  ALOGI("authenticate");
...
ndk::ScopedAStatus
Session::detectInteraction(std::shared_ptr<ICancellationSignal> *out) {
...
ndk::ScopedAStatus
Session::enrollWithContext(const keymaster::HardwareAuthToken &hat,
...
ndk::ScopedAStatus
Session::onPointerDownWithContext(const PointerContext &context) {
  return onPointerDown(context.pointerId, context.x, context.y, context.minor,
                       context.major);
}

ndk::ScopedAStatus
Session::onPointerUpWithContext(const PointerContext &context) {
  return onPointerUp(context.pointerId);
}

ndk::ScopedAStatus
Session::onContextChanged(const common::OperationContext & /*context*/) {
  return ndk::ScopedAStatus::ok();
}

ndk::ScopedAStatus
Session::onPointerCancelWithContext(const PointerContext & /*context*/) {
  return ndk::ScopedAStatus::ok();
}
...
bool Session::isClosed() { return mClosed; }
