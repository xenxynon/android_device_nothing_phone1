...
constexpr common::SensorStrength SENSOR_STRENGTH =
...
Fingerprint::Fingerprint()
    : mDevice(nullptr), mMaxEnrollmentsPerUser(MAX_ENROLLMENTS_PER_USER),
  ...
  if (sensorTypeProp == "" || sensorTypeProp == "default" ||
      sensorTypeProp == "rear")
  ...
  mMaxEnrollmentsPerUser =
      FingerprintHalProperties::max_enrollments_per_user().value_or(
...
fingerprint_device_t *Fingerprint::openHal() {
  ...
  if (hw_get_module_by_class(FINGERPRINT_HARDWARE_MODULE_ID, NULL, &hw_mdl) !=
      0) {
...
void Fingerprint::notify(const fingerprint_msg_t *msg) {
  ...
  if (thisPtr == nullptr || thisPtr->mSession == nullptr ||
      thisPtr->mSession->isClosed()) {
...
ndk::ScopedAStatus Fingerprint::getSensorProps(std::vector<SensorProps> *out) {
  ...
  if (dim.size() >= 3 && dim.size() <= 4) {
    ...
    LOG(WARNING) << "Invalid sensor location input (x|y|radius|display): "
                 << loc;
  }
...
ndk::ScopedAStatus
Fingerprint::createSession(int32_t /*sensorId*/, int32_t userId,
                           const std::shared_ptr<ISessionCallback> &cb,
                           std::shared_ptr<ISession> *out) {
  CHECK(mSession == nullptr || mSession->isClosed())
